...
c
      ...
      integer*4 tip_vinc(maxunits),ii ! tipo di vincolo da rispettare: 
...
c ****************************************INIZIO 2026/03/18********************************************            
             do ij=1, type_quant(itip)
              	qmin_0(ii) = (qmin_assoluto(itip)*lsim_n(ii))
     *             /(1+perc_equi_min(itip))
	            qmin_0_tot=qmin_0_tot+qmin_0(ii)
                 ii=ii+1
             enddo
	    qmin_0_type(itip)=(qmin_assoluto(itip))/(1+perc_equi_min(itip))
          write(LGU,*)'CSUP- qmin_assoluto(itip): ',qmin_assoluto(itip)
          write(LGU,*)'CSUP- perc_equi_min(itip): ',perc_equi_min(itip)
          write(LGU,*)'CSUP- qmin_0_type(itip): ',qmin_0_type(itip)
          write(LGU,*)'CSUP-VALORE INIZIALE flowm: ',flowm
          write(LGU,*)'CSUP-VALORE INIZIALE qmin_0_tot: ',qmin_0_tot
          climite = flowm/qmin_0_tot
          write(LGU,*)'CSUP-VALORE INIZIALE climite: ',climite
c ****************************************FINE  2026/03/18********************************************
...
561   format('Tipo di compressore: ',i3)
      ...
      write(LGU,*)'INIZIO Chiamata portate_criterioP_serie.............'
...
cgpe     -                                   ,type_act_old
     ...
     -								   ,ier)
      ...
      write(LGU,*)'FINE Chiamata portate_criterioP_serie.............'
...
c
      ...
      write(LGU,*)'CSUP-DOPO calc_limiti...'
      do i = 1,ntp
        if (type_actr(i).gt.0) then
          qmin(i) = 0
          qmax(i) = 0
      ...
      do i = 1,ntp
        if (type_actr(i).gt.0) then
          itipo = i
          write(LGU,*)'CSUP-Chiamata q_clim per minimo...',i
...
!            if (.not.iopt) then
     ...
     *               qmin_assoluto(i)*type_actr(i)
          ...
          write(LGU,*)'CSUP-Chiamata q_clim per massimo...',i
...
c ****************************************2026/03/18********************************************
c L'assegnazione di qass viene FORZATA indipendentemente dalle iterazioni mentre hass è frutto dell'ultima iterazione
c *******************************************************************************************
	qass(itip)=climite*qmin_0_type(itip)
c	hass(itip)=hmin_assoluto(itip)
c *******************************************************************************************
      write(LGU,*)'CSUP - climite: ',climite
      write(LGU,*)'CSUP - qmin_0_type(itip):',qmin_0_type(itip)
      write(LGU,*)'CSUP - qass(itip):',qass(itip)
      write(LGU,*)'CSUP - hass(itip):',hass(itip)     
            endif
          enddo
c ****************************************END 2026/03/18********************************************   
      ...
      do itip=1,ntp
        if(type_act_old(itip).gt.0) then
          do ij =1,type_quant(itip)
            icj = nutot+ij
            if(status(icj)) then
              write(LGU,*)'CSUP-VALORE ntp: ',ntp
              write(LGU,*)'CSUP-ITER itip: ',itip
              write(LGU,*)'CSUP-INPUT hass : ',hass(itip)
              write(LGU,*)'CSUP-INPUT qass : ',qass(itip)
              write(LGU,*)'CSUP-VALORE unit_num:',unit_num
...
c    ********* 2026/01/27 NUOVO CONTROLLO SU PRESSIONE DI USCITA FASE 2 MAGGIORE DELLA PRESSIONE DI MANDATA***********
      do itip= 1, ntp
        write(LGU,*)'CSUP-Valore pres_mand input:',pres2
        write(LGU,*)'CSUP-Valore press_out:', press_out_calc(itip) 
        presMandataCalc = press_out_calc(itip) - delpr2
        write(LGU,*)'CSUP-Valore pres_mand calc:',presMandataCalc
...
c
      itipo = itip
c 
      write(LGU,*)'CPB-VALORE itip: ',itip
      write(LGU,*)'CPB-VALORE flowt: ',flowt
      write(LGU,*)'CPB-VALORE qsur_mir: ',qsur_mir(itip)
      write(LGU,*)'CPB-VALORE qcho_mar: ',qcho_mar(itip)
...
c - completamento common
      un_flow1 = flow1
      flow     = flow1/aux3      
      write(LGU,*)'VPB-ZG CALL INPUT revmin:',revmin
      rb = zero_giri(revmin)
      write(LGU,*)'VPB-ZG CALL OUT-MIN:',rb
      write(LGU,*)'VPB-ZG CALL INPUT revmax:',revmax
      ra = zero_giri(revmax)
      write(LGU,*)'VPB-ZG CALL OUT-MAX:',ra
      if (((ra*rb).lt.0)         .or.
     *     (abs(ra).le.eps_giri) .or.
     *     (abs(rb).le.eps_giri) ) then
           write(LGU,*)'VPB-COND-01'
        if ((ra*rb).lt.0 ) then
            write(LGU,*)'VPB-COND-02'
          rd = revmax
cgpe-fun          call zero_fun1(revmin,revmax,rd,rb,ra,rsol,
     ...
     *         epsma,pmach,zero_giri)
          ...
          if (abs(ra).le.eps_giri) then
              write(LGU,*)'VPB-COND-03'
            rev1 = revmax
          else
              write(LGU,*)'VPB-COND-04'
            rev1 = revmin
          endif
        endif
      else if (ra.gt.0) then
        flag = 1
        write(LGU,*)'VPB-COND-flag = 1'
        return
      else
        flag = 2
        write(LGU,*)'VPB-COND-flag = 2'
        return
      end if
c
...
cport     *         head2,rev2,dpres)
     ...
     *         head2,rev2)
          write(LGU,*)'VPB-calc_punto_fase2-flow1:',flow1
          write(LGU,*)'VPB-calc_punto_fase2-head1:',head1
          write(LGU,*)'VPB-calc_punto_fase2-itipo:',itipo
          write(LGU,*)'VPB-calc_punto_fase2-flow2:',flow2
          write(LGU,*)'VPB-calc_punto_fase2-head2:',head2
          write(LGU,*)'VPB-calc_punto_fase2-rev2:',rev2         
          !  dpres non è citato tra i parametri formali!
...
c tra qsur_mir e qcho_mar sulla seconda fase
     ...
     *         qsur2_mir,haux,irange)
          write(LGU,*)'VPB-qdan_su - qsur2_mir:',qsur2_mir
          call QDAN_CH(clim2(4,itipo),qcho2_mar,type2_max_rev(itipo),
     *         chc2(1,itipo))
          write(LGU,*)'VPB-QDAN_CH - qcho2_mar:',qcho2_mar
          if ((flow2.ge.qsur2_mir).and.(flow2.le.qcho2_mar)) then
            call limiti_head2(flow2,hmin2,hmax2,itipo)
            write(LGU,*)'VPB-limiti_head2 - hmin2:',hmin2
            write(LGU,*)'VPB-limiti_head2 - hmax2:',hmax2
...
c calcolo i limiti in portata massimo e minimo
              ...
              write(LGU,*)'VPB-limiti_flow2 - qmin2:',qmin2
              write(LGU,*)'VPB-limiti_flow2 - qmax2:',qmax2
              if ((flow2.le.qmax2).and.(flow2.ge.qmin2)) then
                  write(LGU,*)'VALORE head1: ',head1  
                  write(LGU,*)'VALORE head2: ',head2
                  write(LGU,*)'VALORE rev1: ',rev1
                  write(LGU,*)'VALORE rev2: ',rev2
                  write(LGU,*)'VALORE hmax2: ',hmax2
                  write(LGU,*)'VALORE hmin2: ',hmin2
                  write(LGU,*)'VALORE flow2: ',flow2
                  write(LGU,*)'VALORE qmax2: ',qmax2
                  write(LGU,*)'VALORE qmin2: ',qmin2 
                  write(LGU,*)'VPB-VALORE epsma,pmach: ',epsma,pmach
                  write(LGU,*)'Condizione di iterazione: 
     *            (flow2<=qmax2).and.(flow2>=qmin2)'
...
c     *                        flow1,type_maxtint(itip),temp_int)
      write(LGU,*)'AC CALL 19'
      call single_out_temp_ac_new(tair,pres_int,itip,1,
     *                        flow1,type_maxtint(itip),temp_int
     *                        ,pin,tin,deltat)
...
c flow_tot = parte di portata che deve essere ancora considerata ai fini della simulazione dei TC
	flow_tot = flowm - flow_ass
      write(LGU,*)'PORTSER-Valore flow_tot:',flow_tot
      write(LGU,*)'PORTSER-Valore qmin_tot:',qmin_tot
      write(LGU,*)'PORTSER-Valore qmax_tot:',qmax_tot
c
c-l_ric      l_ric = .true.
     ...
     *          vinc_min(itip) .ne. sur_2f     ) then
        ...
        do itip = 1,ntp
          if (type_actr(itip).gt.0) then
            ...
            write(LGU,*)'PORTSER-Valore itip:',itip
            write(LGU,*)'PORTSER-qass=qmin_assoluto:',qass(itip)
            write(LGU,*)'PORTSER-hass=hmin_assoluto:',hass(itip)
          endif
        enddo
...
cgpe-new        end if
        ...
        do itip=1,ntp
          if (type_actr(itip).gt.0) then
            ...
            write(LGU,*)'PORTSER-Valore itip:',itip
            write(LGU,*)'PORTSER-qass=qmax_assoluto:',qass(itip)
            write(LGU,*)'PORTSER-hass=hmax_assoluto:',hass(itip)
          endif
        enddo
...
c
      write(LGU,*)'PORTSER-CALL trattaFuoriMappa_serie....'
	...
	flow_tot = flowm-flow_ass
      write(LGU,*)'PORTSER-Valore flow_tot:',flow_tot
      write(LGU,*)'PORTSER-CALL trattaNormale_serie....'
...
c zin e' nel common compstat.INC
     ...
     *                    ,itip,portata)
	    ...
	    !================ aree common ==================
          ...
          itipo = itip
          write(LGU,*)'ZG CALL 03'
          rb = zero_giri(rev_min)
          write(LGU,*)'ZG CALL 04'
...
cgpe-fun              call zero_fun1(rev_min,rev_max,rd,rb,ra,rsol,
     ...
     *                  epsma,pmach,zero_giri)
          ...
          write(LGU,*)'TNS-VALORE itip:',itip
          write(LGU,*)'TNS-CALL qdan_ch-VALORE qaux:',qaux
          write(LGU,*)'TNS-VALORE flow_unit:',flow_unit
          write(LGU,*)'TNS-VALORE rev:',rev
          if (flow_unit.le.qaux) then
            call hdaqn(haux,flow_unit,rev,chn(1,itipo))
            write(LGU,*)'TNS-PREVALENZA CON chn:',haux
          else
            call hdaqn(haux,flow_unit,rev,chc(1,itipo))
            write(LGU,*)'TNS-PREVALENZA CON chc:',haux
          end if
          qass(itip)      = flow_unit
          hass(itip)      = haux
	    flowTot = flowTot - portata*type_actr(itip)
